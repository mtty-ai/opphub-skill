 ...
 *
 * v3.1.0-alpha.3.2 (舟哥 15:00 拍 "偶合登出不能用"):
 *   alpha.3 14:25 改时漏了 import clearToken, logout 子命令报 'clearToken is not defined'
 *   修: 代理补 clearToken
 */
 ...
 *
 * v3.1.0-alpha.3.1 (舟哥 14:29 抓的 bug fix):
...
export async function refreshToken({ force = false } = {}) {
  ...
  if (force && status === "valid") {
    // v3.1.0-alpha.3.1 (舟哥 14:29): 临时把 expires_at 设为 1 (已过期), 不动 Keychain
    // getAccessToken 看到 status=expired 但 refresh_token 还有效, 走 refresh 路径
    const fakeT = { ...t, expires_at: 1 };
    const statusFake = client.tokenStatus(fakeT);  // → 'expired' (refresh_token 还在)
    // 但 getAccessToken 接受的是 Keychain 里的 t, 不是 fakeT
    // 真正要 trigger refresh, 需调用 awaitRefreshToken (private function)
    // fallback: 直接 fetch refresh endpoint 自己拿新 token 写 Keychain
    return await directRefreshAndWrite(client, t);
  }

  // 路径 3: valid (不强制) → 不需要 refresh
  if (status === "valid") {
    return t.access_token;
  }

  // 路径 4: expired/missing → 返 false, 让 bot 走 device flow
  throw new Error(`token status=${status}, refresh_token 也过期, 需要重新 device flow`);
}

/**
 * v3.1.0-alpha.3.1 (舟哥 14:29 bug fix):
...
async function directRefreshAndWrite(client, t) {
  ...
  const TOKEN_URL = process.env.OPPHUB_OAUTH_TOKEN ||
