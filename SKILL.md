...
requires:
  ...
  platform:
    ...
    windows: none             # 暂不支持
  # v3.1 (维护者 13:15 + 13:18 钉): skill 调用 LLM 工具 + 本机 memory + wiki 作为数据源
  tools:
    - web_search              # 联网搜公司信息 (维护者 13:15) - 统一命名
    - web_fetch               # 拉 URL (知乎/小红书/公众号/官网)
    - understand_image        # 解析用户上传图片
    - pdf                     # 解析 BP / 案例集 PDF
    - memory_search           # 搜本机 memory (维护者 13:18)
    ...
    # - bot.skillApi.send / askInteractive / askFreeText / getChannel (维护者 13:28 钉)
metadata:
  ...
  # v3.1 (维护者 13:28 钉): skill 不拼原生 channel card, 走 OpenClaw runtime 渲染层
  channelRenderer:
...
- 装完 skill 默认没建 cron, 维护者不说话 bot 不动
...
> 7/17 16:58 维护者拍 "skill 没显示选中通道" → 修了
...
- 验证码**不在公开对话框贴**(runtime 12:04 误贴 567425 教训)
...
### 🚨 红线 (维护者 7/20 13:35 钉死): 不允许 skill 编写和瞎猜

> **事件**: 7/20 13:31 我没联网,凭 daily memo 残影编了"创始人=维护者 / 业务群=A/B/C",**公司名称都没验证**。
>
...
bot 走 `bot.skillApi.askInteractive` 让维护者拍:
...
- ⏳ 阶段 4 UI 提醒 (跟能力画像同款)
- ⏳ server 端 source_type: card-ability / card-upstream / card-downstream / card-peer (维护者未拍)
...
## 🔒 v3.3 idempotent ingest · 幂等入库 + 去重 + 冲突处理（维护者 7/20 17:30 拍）
...
{ "ok": false, "conflict": true, "conflictReport": { "entryId": "cmxxx", "conflictFields": ["legal_person: 刘会冬 → 张维护者"], ... } }
```
...
按 v3.1 §阶段 4 设计, bot 用 `IntentMessage.askInteractive` 让维护者拍冲突项:
```
...
维护者 7/20 12:58 拍"字段概念彻底不要, 整段自由文本进知识库"—— 但 **去重键必须 1 个结构化锚点**:
