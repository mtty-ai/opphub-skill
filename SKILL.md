...
| 偶合退出 | `bin/opphub-oauth-login logout` | **E3 修复**: SKILL.md 原来写 `opphub-oauth-logout` 是 bin, 实际不存在; oauth-login 里子命令 `logout` 才是入口 (oauth-login.js:483 `if (cmd === "logout")`). 清 Keychain,下个命令强制重新 device flow |
| 偶合状态 | `bin/opphub-oauth-login status` | **E2 修复**: SKILL.md 原来写 `opphub-status` bin, 实际不存在; oauth-login 里子命令 `status` 才是入口 (oauth-login.js:425). 读 Keychain + tokenStatus 状态机 + plugin_oauth_client.ok + cron_check (skill 自带 cron) |
...
>
> 1. **不联网 = 不准答业务信息** — 阶段 1 没真查 `web_search` / `web_fetch`,就返 `validation.ok=false` + `error=not_searched`,**绝不** 拿本机 memory / workspace 文件凑答案
...
| # | 数据源 | 工具 |
|---|---|---|
| 1 | 联网搜工商 | `web_search` + `web_fetch` |
| 2 | 联网搜新闻 / 媒体 | `web_search` |
| 3 | 联网搜招聘 | `web_search` |
| 4 | 联网搜项目 / 客户 | `web_search` |
...
**不存在 `for card in "${cards[@]}"; do opphub-knowledge-add --raw-text ...`** 姿势 skill-only, 不循环 add
